...
export default class turntable {
  ...
  constructor(element, config) {
    ...
    this.btnBgUrl = config.btnBgUrl || 'https://p0.ssl.qhimg.com/t01d5de9bc40a2417ad.png'
    ...
    this.bgUrl = config.bgUrl || 'https://p3.ssl.qhimg.com/t01ac8f01769d101d1b.png'
  ...
  init() {
    const { reward, btn_bg_color, btn_color, btn_txt, borderColor, bgUrl, width, height } = this
  ...
  drawBg({ borderColor, bgImg }) {
    ...
    if (bgImg) {
      ctx.save()
      ctx.translate(-x, -y)
      ctx.drawImage(bgImg, 0, 0, bgImg.width, bgImg.height, 0, 0, width, height)
      ctx.restore()
    }
    ctx.restore()
  }
  ...
  drawFan({ reward }) {
    ...
    reward.map((item, index) => {
      ...
      let img_x = -targetW / 2
      let img_y = 30 - r
      ctx.drawImage(img, 0, 0, img.width, img.height, img_x, img_y, targetW, targetH)
  ...
  drawImg() {
    ...
    r_ctx.drawImage(this.canvas, 0, 0, this.width, this.height, 0, 0, this.width, this.height)
    r_ctx.drawImage(
      this.btn_canvas,
      0,
      0,
